...
class TransactionBuilder(dict):
    ...
    def list_operations(self):
        ...
        if self.hive.is_connected() and self.hive.rpc.get_use_appbase():
            ...
            appbase = False
        return [Operation(o, appbase=appbase, prefix=self.hive.prefix) for o in self.ops]
    ...
    def json(self, with_prefix=False):
        ...
        json_dict = dict(self)
        if with_prefix:
            json_dict["prefix"] = self.hive.prefix
        return json_dict
    ...
    def appendSigner(self, account, permission):
        ...
        if self.hive.wallet.locked():
            raise WalletLocked()
        if self.hive.use_sc2 and self.hive.hiveconnect is not None:
            self.hive.hiveconnect.set_username(account["name"], permission)
            return
    ...
    def sign(self, reconstruct_tx=True):
        ...
        # presented wrongly!
        if self.hive.rpc is not None:
            operations.default_prefix = (
                self.hive.chain_params["prefix"])
        elif "blockchain" in self:
            operations.default_prefix = self["blockchain"]["prefix"]

        try:
            signedtx = Signed_Transaction(**self.json(with_prefix=True))
            signedtx.add_custom_chains(self.hive.custom_chains)
        except:
        ...
        signedtx.sign(self.wifs, chain=self.hive.chain_params)
    ...
    def verify_authority(self):
        ...
        try:
            ...
            if self.hive.rpc.get_use_appbase():
                ...
                args = self.json()
            ret = self.hive.rpc.verify_authority(args, api="database")
            if not ret:
    ...
    def get_potential_signatures(self):
        ...
        if self.hive.rpc.get_use_appbase():
            ...
            args = self.json()
        ret = self.hive.rpc.get_potential_signatures(args, api="database")
        if 'keys' in ret:
            ret = ret["keys"]
        return ret
    ...
    def get_transaction_hex(self):
        ...
        if self.hive.rpc.get_use_appbase():
            ...
            args = self.json()
        ret = self.hive.rpc.get_transaction_hex(args, api="database")
        if 'hex' in ret:
            ret = ret["hex"]
        return ret
    ...
    def get_required_signatures(self, available_keys=list()):
        ...
        if self.hive.rpc.get_use_appbase():
            ...
            ret = self.hive.rpc.get_required_signatures(args, api="database")
        else:
            ret = self.hive.rpc.get_required_signatures(self.json(), available_keys, api="database")
    ...
    def broadcast(self, max_block_age=-1):
        ...
        try:
            ...
            if self.hive.use_sc2:
                ...
                self.hive.rpc.broadcast_transaction(
                    args, api=broadcast_api)
        except Exception as e:
